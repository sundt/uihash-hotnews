...
class TestURLValidation:
    ...
    def test_validate_http_url_valid_https(self):
        """有效的 HTTPS URL 应通过验证"""
        from hotnews.web.rss_proxy import validate_http_url
    ...
    def test_resolve_and_validate_host_blocks_localhost(self):
        """localhost 应被阻止"""
        from hotnews.web.rss_proxy import resolve_and_validate_host
    ...
    def test_whitelist_allows_domain(self):
        """白名单中的域名应被允许"""
        from hotnews.web.rss_proxy import _get_allowed_domains

        # 设置白名单
        with patch.dict(os.environ, {"HOTNEWS_RSS_ALLOWED_DOMAINS": "allowed.com,trusted.org"}):
            # 重新加载
            import hotnews.web.rss_proxy as proxy
    ...
    def test_get_allowed_ports_default(self):
        """默认应只允许 80 和 443 端口"""
        from hotnews.web.rss_proxy import _get_allowed_ports
    ...
    def test_custom_ports_allowed(self):
        ...
        with patch.dict(os.environ, {"HOTNEWS_RSS_ALLOWED_PORTS": "80,443,8080,9000"}):
            ports = _get_allowed_ports()
            assert 80 in ports
            assert 443 in ports

...
def setup_logger(
    ...
    Args:
        ...
        log_file: 日志文件路径，默认 output/logs/hotnews.log
        console: 是否输出到控制台
    ...
    # 确定日志级别
    if level is None:
        level = os.environ.get("HOTNEWS_LOG_LEVEL", "INFO").upper()
...
def get_logger(name: str) -> logging.Logger:
    ...
    Returns:
        logger 实例
    """
    return logging.getLogger(f"hotnews.{name}")
...
def init_default_logger():
    """初始化项目默认日志配置"""
    log_dir = os.environ.get("HOTNEWS_LOG_DIR", "output/logs")
    log_file = os.environ.get("HOTNEWS_LOG_FILE", f"{log_dir}/hotnews.log")
